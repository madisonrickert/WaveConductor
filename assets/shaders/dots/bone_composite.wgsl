// Additive bone-glow composite for the Dots (Fabric) sketch.
//
// Samples the off-screen bone image (emissive wireframe bones on a black
// background, rendered by `DotsHandMeshCamera3d` with no bloom and no tonemapping)
// and the current scene (the explode-post-processed dots grid), and outputs their
// linear-HDR sum.
//
// This node is inserted into the Core2d graph AFTER the explode post-process and
// BEFORE `Node2d::Bloom`, so the main camera's `Bloom` + `AgX` tonemap then
