// Unlit flat-color material for the Line sketch's hand-mesh wireframe bones.
//
// This is the shader hook for the bone overlay: the bones are real 3D meshes
// (LineList topology) drawn by the off-screen-compositing `HandMeshCamera3d`,
// so arbitrary per-bone shading and downstream post-process passes are all
// possible from here — unlike Bevy's built-in gizmos, whose pipeline shader is
// fixed. Today it simply emits the uniform `color`; extend `fragment` (and add
// bindings to `BoneWireframeMaterial`) for richer effects.
//
