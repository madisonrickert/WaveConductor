...
- `cargo xtask soak-test` runs the app under representative load (hand tracking + audio active, sketch cycling), samples RSS / FPS / thermal tier on a fixed interval, fits leak and decay trends, and writes a self-describing artifact to `target/soak/<label>/` (`run.json` + `samples.ndjson` + `app.log` + the app's last `health.json`). Build the app first (`cargo build -p waveconductor`); soak-test launches the pre-built debug binary and fails fast with that directive if it is missing, exactly as `capture` does.
  ...
  - Smoke the harness first: `cargo xtask soak-test --duration 2m --sample 5s --cycle 20s --warmup 10s`.
...
- Three verdicts, and the distinction is deliberate. **PASS** = ran to completion, no RSS trend, no FPS decay, no freeze. **FAIL** = a mechanically-certain failure (the app died, froze, RSS climbed on a steep well-fit slope, or FPS decayed past the threshold); exits nonzero. **REVIEW REQUIRED** = the data is real but the judgment is not mechanical (a mild upward drift that could be a bounded cache settling, or too few samples to fit); the tool prints the numbers and a directive naming the artifact, and does **not** fabricate a verdict. When you see it, read `samples.ndjson` and judge — that is the intended agent-in-the-loop step, and there is no LLM API spend anywhere in the harness.
- The app-side half is `WC_SOAK` (`wc-core`'s `soak` module): it republishes a `health.json` snapshot, advances the sketch on the cycle interval, and self-exits at the duration. Like `capture`, it is `#[cfg(debug_assertions)]`-gated, so the soak measures a **debug** binary — the leak and stall failure modes it hunts are structural and appear in either profile, and instrumenting release would put per-frame work in the shipped binary. RSS is measured from outside the process (`ps`), so no dependency was added; **GPU memory is not sampled** — watch it by hand in Activity Monitor / `nvtop` if a run's verdict turns on VRAM.
