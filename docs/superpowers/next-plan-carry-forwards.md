...
## From Plan 11.6 (2026-05-28)

74. **HandMesh port for Dots, Cymatics, Waves.** The Line `LineHandMeshPlugin` (Plan 11.6 Phase 13) sets up `HAND_MESH_LAYER` + `HandMeshCamera3d` and spawns 20 wireframe-sphere bone children on every `TrackedHand` while Line is active. Other sketches will want the same mechanism, possibly with sketch-specific materials — cymatics uses orange in v4, waves uses background-derived hue. Land alongside each sketch's port plan rather than upstream-extracting the plugin prematurely.

75. **HandMesh wireframes don't go through Camera2d's HDR/bloom/AgX pipeline.** Phase 13 explicit scope punt: the `HandMeshCamera3d` renders at order=1 on top of `Camera2d` with `ClearColorConfig::None`, but each camera has its own intermediate HDR texture — they aren't sharing a swap target. Wireframes are visible at full brightness without bloom rolloff. Bringing them under the same post-process chain requires a shared `Image` render target + a final blit camera; non-trivial render-graph work and worth its own small plan.

76. **Provider fusion policy.** `fuse_hand_frames` is a trivial passthrough today — the per-hand `FusedHand.provider` tag is correctly populated but the fusion step itself just copies frames. The first plan to register a second provider (likely MediaPipe webcam) needs the per-chirality precedence policy from the design spec: Primary > Simulator role, Leap > MediaPipe among Primary, one fused hand per chirality.

77. **Cymatics center-holding semantics + Waves two-hand-pair math.** Each sketch's port plan will translate v4's per-sketch `onFrame` body. The `TrackedHand` entity model + `HandId` stability (Plan 11.6 Phases 4 + 6) are now in place to support both directly. Cymatics: a held grab keeps the center anchored at the grab position. Waves: needs the distance + chord-angle between Left and Right hands as a continuous parameter.

78. **`LeaprsProvider` declares `unsafe impl Send + Sync` on its `Option<leaprs::Connection>` field.** Needed for Bevy's `Resource` trait (workspace `unsafe_code = "deny"` lint allows item-level `#[allow(unsafe_code)]` with justification, which this uses). The safety claim rests on LeapC's documented thread-safety for a single owning thread. Worth a paragraph in a future audit doc explaining what Bevy actually does with this Resource across ticks; if the executor ever migrates the resource across threads concurrently, the assumption breaks.

79. **`MockProvider` doesn't override `as_any_mut`.** Plan 11.6 Phase 10.3 added the trait method (default-impl returns `None`) only for `LeaprsProvider` so the `apply_leap_background_setting` system can downcast and call typed methods. Tests that want to drive scripted frames into an already-registered `MockProvider` via the registry have to use `MockProvider::with_frames(...)` upfront — there's no path to `push_frame` post-registration. Add the override + a typed extension if a future plan needs live mock frame injection.

80. **HandMesh observer's `Assets<Mesh>` / `Assets<StandardMaterial>` params are `Option<ResMut<>>`.** Phase 13 wrapped them in `Option` so the `MinimalPlugins`-based test harness doesn't panic when the observer fires for synthetic `TrackedHand` spawns. Production code paths assume the assets exist. Either document the assumption or move to non-Option params and have the test harness opt in to `AssetPlugin` for any test that spawns hands.

81. **`vendor/leapc/` filename version is 6, not 5.** The SDK 6.2.0 installers ship `libLeapC.6.dylib` / `libLeapC.so.6` (soname version 6) — the plan's draft assumed `.5` from an older SDK convention. `.cargo/config.toml` rpath stanzas use directory paths so are unaffected, and unversioned symlinks `libLeapC.dylib` / `libLeapC.so` exist alongside the versioned files for `-lLeapC` resolution. When the next SDK refresh lands, `vendor/leapc/README.md`'s refresh procedure should mention re-creating the symlinks.

82. **leaprs API surface vs the plan's draft.** Phase 8 implementer noted several plan-draft API names that needed correction against the real upstream `plule/leaprs 0.2.2`: `hand.pinch_strength` / `hand.grab_strength` are fields via Deref (not methods); `EventRef::DeviceLost` is a unit variant (no payload); `tracking.hands()` returns a `Vec<HandRef>` (not a slice); the underlying `LEAP_FRAME_HEADER::timestamp` is `i64` microseconds; device info requires `dev.device().open()` then `.get_info()` rather than a direct `.info()` on the event ref. Future leaprs version bumps may shift these again — re-verify against the source under `/tmp/leap-sys-inspect/leaprs-<ver>/src/` or the upstream repo.

83. **Pre-existing clippy backlog cleared inline during Plan 11.6 Phase 0.** Commit `9f47914a` fixed `field_reassign_with_default` + `float_cmp` lints in `crates/wc-core/src/lifecycle/reload.rs` (test mod) and `doc_markdown` lints in `crates/waveconductor/src/main.rs` (HDR doc block) so the per-task clippy gates throughout 11.6 could pass. These were introduced by Plan 11.5's UI work and Plan 11.5's HDR addition — separate plans, fixed in this one as the only way to keep workspace `-D warnings` green.

## From 2026-05-30 performance-research triage

Source: an external Perplexity research pass on a thermal / self-throttling stack for the 12+ hour unattended kiosk, triaged + adversarially reviewed (senior-engineer subagent). The strategic framing and full ranked list live in `roadmap.md` → *Performance & reliability hardening*; these are the small, well-scoped items that fold into a Phase 0. Tags: **[now]** cheap+compounding or a real bug · **[before-pi]** gates the unattended run · **[defer]** needs full-render soak evidence first.

84. **[before-pi] Wire the Leap idle-pause systems — a claimed-shipped thermal lever that is dead code.** `pause_leap_on_screensaver` (`crates/wc-core/src/input/providers/leap_native.rs:693`) and `resume_leap_on_active` (`:702`) are defined and documented as screensaver "Seam 4," but **registered nowhere** — verified 2026-05-30, the only references in `crates/` are the two definitions plus one doc-comment; the whole `set_paused → set_all_leap_paused → pause_leap_on_screensaver` chain is unreachable. So the Ultraleap service runs at full power through the entire idle screensaver — on a box whose design goal is *not thermally collapsing over 12 hours*. The roadmap previously asserted this seam "shipped" and "reliably sheds host CPU" (now corrected). Fix: register `pause_leap_on_screensaver` on `OnEnter(SketchActivity::Screensaver)` and `resume_leap_on_active` on `OnEnter(SketchActivity::Active)`, behind the same leap / `hand-tracking-gestures` cfg that gates `apply_leap_background_setting` in `crates/wc-core/src/input/mod.rs`. ~6 LOC + a wiring test. Highest-ROI item in the perf stack.

85. **[shipped 2026-05-30] `bevy/dynamic_linking` behind a dev-only flag/alias (spike first).** Large incremental-link win on the Bevy dep tree. Must stay **out of release** (incompatible with `lto="fat"`/`strip`) **and out of WASM** (no dylib). Enable via `--features bevy/dynamic_linking` on a dev cargo alias, not the manifest default, so it can't leak into CI/release/web. **Spike against the vendored-LeapC rpath** in `.cargo/config.toml` first: with `dynamic_linking` on, dev runtime needs *both* `libbevy_dylib` (Cargo-resolved) and `libLeapC` (`$ORIGIN`-relative rpath) — should coexist, but test rather than assert.
    - **As-built:** `cargo rund` alias in `.cargo/config.toml` (`run --package waveconductor --features bevy/dynamic_linking`). Passed as a CLI feature, *never* a crate `[features]` entry — verified `cargo tree --all-features` pulls **0** `bevy_dylib` (so CI's `--all-features` clippy/test/deny/doc can't leak it; default build likewise 0). Coexistence spike on Apple Silicon confirmed `otool -L` references both `libbevy_dylib` and `@rpath/libLeapC.6.dylib`, and a `cargo rund` launch initialized the Metal renderer, window, **and `LeaprsProvider`** with no dyld errors. Gotcha recorded in the alias comment: the dylib binary is not self-contained (libstd is also dynamically linked), so it must be launched via `cargo rund`, not the bare `target/debug` binary. Fast-linker half (#86) still open.

86. **[now] Fast linker (mold/lld) via the existing per-target `rustflags`.** `.cargo/config.toml` currently has rpath flags only, no linker override. Append `-C link-arg=-fuse-ld=mold` (Linux build/CI; `lld` as the safe fallback) to the **existing** per-target `rustflags` array — merge, don't replace, or you drop the LeapC rpath. **Skip `sold`/`zld` on macOS:** Apple's `ld` (ld-prime) is already fast in 2026 and `lld`/Mach-O has had edge cases; measure the default first.

87. **[before-pi] `DefaultPlugins` full-render, screensaver-resident soak on the candidate device (NUC *or* iPad).** The existing 8-hour soak (`crates/wc-sketches/tests/line_soak.rs`) runs `MinimalPlugins` — no RenderApp, no GPU — so the real renderer has *never* run unattended on hardware. This soak is the gating prerequisite for the `PerformanceGovernor` / DRS work (it either proves the present-rate throttle already holds thermals → most of the perf stack is YAGNI, or produces the frame-time + temperature data that justifies a governor). On the NUC it also tunes the placeholder °C thermal bands (`enter_warm 75` / `enter_hot 90` in `lifecycle/thermal/mod.rs`); on a fanless A12Z iPad it is the GPU/thermal go/no-go for the whole iOS target (the multi-pass HDR + gravity-post + bloom + blur stack at 2388×1668 is the #1 mobile-GPU risk). The one artifact that serves both candidate devices. Supersedes/clarifies the existing screensaver "8-hour `DefaultPlugins` soak" carry-forward.

88. **[before-pi, with #87] CSV / structured soak telemetry.** The soak currently logs only end-of-run `elapsed + update count` to tracing. Add a periodic structured row (timestamp, frame-time mean/p95/p99, `ThermalState.tier` + `last_temp_c`, entity count) to a CSV so the full-render soak is analyzable instead of vibes. Overlaps the Pre-release "Perf audit harness" item — build once, used by both.

89. **[before-pi] Unattended crash recovery — device-specific.** **Linux / NUC:** a systemd `.service` unit (in the deploy/provisioning assets, not the app) with `Restart=always`, plus `WatchdogSec=` and an `sd_notify(WATCHDOG=1)` ping from the main loop. **iOS / iPad:** there is *no* `Restart=always` — Guided Access does not relaunch a crashed app and doesn't survive reboot; the real path is **MDM / Apple Configurator Single App Mode on a supervised device** (`com.apple.app.lock`), which relaunches on reboot (though even ASAM doesn't cleanly re-lock across crash+restart). Mostly ops / provisioning, not app code. **Explicitly rejected on both:** an in-app supervisor thread — fleet-scale weight for a one-box deploy. (Linux CPU governor / driver pinning are likewise OS-image / `cpupower` provisioning concerns, not app code.) **Update 2026-06-03:** crash/dependency recovery is now folded into the `leap-deep-idle-state` item and its design doc (`specs/2026-06-03-leap-service-recovery-design.md`), which treats **all three desktop OSes as first-class** (macOS launchd `KeepAlive` + `SMAppService` helper, Linux systemd + polkit, Windows SCM recovery + ACLs) — not just Linux/iPad as framed above.

## From 2026-05-30 iPad/iOS target triage

Source: the iOS / iPad research triage (see `roadmap.md` → *iOS / iPad deployment target*). These activate only if iPad is chosen as a (or the) deployment device — except #90, which is a device-agnostic design fix.

90. **[before-pi] Generalize the thermal sensor seam from °C-only to source-agnostic (tier signal).** The whole thermal pipeline is °C-typed end to end — `TemperatureSensor::read_celsius -> Option<f32>`, `ThermalReading { temp_c: f32 }`, `ThermalThresholds::classify(current, temp_c)`. That's fine for the Linux sysfs reader and macmon, but **iOS exposes no °C** — only `ProcessInfo.thermalState` (a 4-level OS enum). The `ThermalState` *resource* already absorbs a new source (add `ThermalSource::OsThermalState`, `last_temp_c: None`), but the *sensor abstraction* does not: an iOS backend must map the enum → `ThermalTier` **directly, bypassing `TemperatureSensor` and `ThermalThresholds`** (the OS already applies hysteresis; synthesizing fake °C to feed the classifier is a hack that re-derives what the OS gives you). Introduce a second ingestion shape at the sampler boundary (a `tier`-producing source alongside the `°C`-producing one). Worth doing **regardless of which device wins** — it's the source-agnostic generalization the v5 thermal thesis rests on, and it's the design's real generalization test. ~`ThermalState`/`sensor.rs` refactor + an iOS `objc2` `thermalState`-notification backend.
