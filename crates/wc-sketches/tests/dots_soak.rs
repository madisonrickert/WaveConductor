...
fn dots_soak_8h() {
    ...
    // Enter Dots via Digit3 keyboard nav. Three updates is the minimum for
    // `KeyboardInput` → `ButtonInput<KeyCode>` → `emit_action_input` emits
    // `ActionInput` → `NextState` to settle (see `dots` input wiring in
    // `wc_core::lifecycle::action_map`).
