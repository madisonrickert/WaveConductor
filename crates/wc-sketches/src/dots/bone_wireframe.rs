//! Metal-safe wireframe-bone material + geometry for the Dots hand-mesh overlay.
//!
...
//! custom [`DotsBoneWireframeMaterial`]. Line primitives need no special wgpu
//! feature.
//!
//! Because the bones are real 3D meshes drawn by the off-screen-compositing
//! `DotsHandMeshCamera3d`, this material — and any post-process pass added to
//! that camera's graph — is the project's hook for applying shaders/effects to
//! the Dots bones. (Bevy's built-in gizmos can draw Metal-safe lines too, but
//! their pipeline shader is fixed and accepts no custom material, so they
//! cannot meet that requirement.) See `assets/shaders/dots/bone_wireframe.wgsl`.
//!
//! ## Carry-forward
//!
//! This material, mesh, and shader are generic and duplicate
//! [`crate::line::bone_wireframe`]. A shared home under `wc-core` or a
//! `particles/` sub-crate is the eventual move; for now each sketch carries
//! its own copy so they can diverge independently (different default colors,
//! future per-sketch shader extensions).
...
/// Path to the Dots bone wireframe WGSL (vertex + fragment), relative to the
/// asset root.
const BONE_WIREFRAME_SHADER: &str = "shaders/dots/bone_wireframe.wgsl";
...
/// Unlit flat-color material for the Dots wireframe bones.
///
...
///
/// Carry-forward: generic; duplicates [`crate::line::bone_wireframe::BoneWireframeMaterial`].
/// Both move to a shared home when the sketch count justifies it.
#[derive(Asset, AsBindGroup, TypePath, Debug, Clone)]
pub struct DotsBoneWireframeMaterial {
    ...
    /// `crate::dots::hand_mesh::BONE_GLOW_INTENSITY`) so the HDR overlay's bloom
...
impl Material for DotsBoneWireframeMaterial {
...
///
/// Carry-forward: identical to [`crate::line::bone_wireframe::icosphere_line_mesh`];
/// the two share geometry until they move to a common module.
#[must_use]
...
mod tests {
    ...
    /// Verifies that `icosphere_line_mesh` produces a `LineList` mesh with
    /// paired indices and the expected minimum vertex count.
    ///
    /// Adapted from the equivalent test in [`crate::line::bone_wireframe`];
    /// the underlying geometry function is identical.
    #[test]
