...
//! pinned to 1.0 while the mouse attractor is active. The audio coupling
...
//! envelope, which saturates and made hand audio feel binary on/off.
...
/// Z or farther gets distance attenuation 0.0 (silent). Matches the 350 mm
/// far plane of the visual power model's depth modulator in
/// `update_line_hand_attractors`.
pub const HAND_DRIVE_Z_FAR_MM: f32 = 350.0;

/// Release time constant τ (seconds) for [`HandAudioDrive`] when its target
/// drops below the current value. Chosen to match the synth envelope's
/// hand-tuned release (`grouped_upness_release` ≈ 670 ms in
/// [`super::particle_stats::EnvelopeRates::default`]) so a just-released
/// grab's volume tail is shaped by the envelope, never clipped by the drive
/// snapping to zero. Rising targets are applied instantly — closing the fist
/// must be audible at once, mirroring the envelope's fast attack.
pub const HAND_DRIVE_RELEASE_TAU_S: f32 = 0.67;
...
/// far hands are proportionally quieter); mouse interaction pins it to 1.0
/// so click-driven audio keeps its historical loudness.
...
/// to 1.0 so click-driven audio keeps its historical loudness. Rising targets
/// apply instantly; falling targets decay with [`HAND_DRIVE_RELEASE_TAU_S`]
/// so a just-released grab's volume tail isn't clipped (see
/// [`step_hand_audio_drive`]).
///
...
fn update_hand_audio_drive(
    ...
    drive.0 = step_hand_audio_drive(drive.0, target, time.delta_secs());
}
...
///
/// Mouse interaction has no grab/depth axes, so it pins the target to full
/// drive — click-driven audio keeps its pre-drive loudness. When neither
/// input is active the target is 0.0 and [`step_hand_audio_drive`]'s release
/// decay (not a snap) takes the drive down while `grouped_upness` releases.
...
/// rise, exponential decay with τ = [`HAND_DRIVE_RELEASE_TAU_S`] on fall.
///
...
pub fn step_hand_audio_drive(current: f32, target: f32, dt: f32) -> f32 {
    if target >= current {
        ...
        current + (target - current) * (dt / HAND_DRIVE_RELEASE_TAU_S).min(1.0)
    }
}
...
mod tests {
    ...
    fn rising_target_applies_instantly() {
        ...
        assert_eq!(step_hand_audio_drive(0.2, 0.9, 0.016), 0.9);
        assert_eq!(step_hand_audio_drive(0.0, 1.0, 0.016), 1.0);
    }
    ...
    fn falling_target_decays_without_snapping() {
        ...
        let dt = 0.016;
        let stepped = step_hand_audio_drive(1.0, 0.0, dt);
        let expected = 1.0 - dt / HAND_DRIVE_RELEASE_TAU_S;
        assert!(
    ...
    fn release_converges_to_target() {
        // Integrate ~3 s of 60 fps frames: drive must land near zero (tail
        // fully released) without going negative.
        let mut drive = 1.0;
        for _ in 0..180 {
            drive = step_hand_audio_drive(drive, 0.0, 1.0 / 60.0);
        }
        assert!(drive >= 0.0);
        assert!(
            drive < 0.02,
    ...
    fn system_writes_drive_from_tracked_hand() {
        ...
        // so the τ = 0.67 s release path applies — integrate ~3 s of 60 fps
        // frames (≈ 4.5τ) and require convergence near 0.5.
