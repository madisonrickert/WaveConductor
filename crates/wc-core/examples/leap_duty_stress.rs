...
/// Run one duty-cycle config for `secs`. Returns
/// `(cycles, cycles_with_a_hand_frame, total_frames, wedged)`.
fn run_config(
    p: &mut LeaprsProvider,
    m: &mut Messages<HandTrackingFrame>,
    window: Duration,
...
fn main() {
    ...
    for (window, gap) in configs {
        ...
        println!(
            "── config: window={wms}ms gap={gms}ms (period {}ms) for 20s ──",
            wms + gms
        );
        ...
        let pct = (with_hand * 100).checked_div(cycles).unwrap_or(0);
        let avg = if cycles > 0 {
            f64::from(total) / f64::from(cycles)
        } else {
            0.0
        };
        println!(
            ...
            if wedged {
                "WEDGED (recovery probe saw no frame in 3s)"
            } else {
                "device alive"
            },
        );
