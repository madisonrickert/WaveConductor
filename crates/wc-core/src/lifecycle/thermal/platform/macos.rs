//! `macmon`-backed temperature sensor for Apple Silicon.
//!
//! `sysinfo` returns an empty `Components` list on Apple Silicon, so the macOS
//! path reads the `SoC` temperature through `macmon`'s `IOReport`/`IOKit`
//! sampler (no sudo). `macmon` is a young library wrapping a *private* Apple
//! API, so it is pinned (`=0.7.0` in the workspace manifest) and fully wrapped
...
/// keep the background thread responsive, long enough for `IOReport` to produce
/// a stable reading.
...
impl TemperatureSensor for MacmonSensor {
    ...
    /// (the `SoC` shares a thermal envelope). Any sampling error or a
    /// non-finite / zero reading (macmon reports `0.0` when a sensor channel is
    /// unavailable) yields `None`.
